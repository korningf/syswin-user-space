...
namespace syswin
{
    public static class ProcessInfoTools
    {
        ...
        {
			Console.WriteLine("ShowTerminal({0}) called", directory);
            try
            {
                if (string.IsNullOrEmpty(directory))
                {
					Console.WriteLine("Warning, directory is empty - assume function failed");
                    return false;
                }
                ...
                using (Process p = Process.Start(startInfo))
                {
                    if (p == null)
                    {
                        Console.WriteLine("Warning, Process.Start() returned null, assuming function failed");
                        return false;
                    }
                }
                return true;
            ...
            {
				Console.WriteLine(string.Format("Unable to bring up CMD.EXE in {0}", directory), e);
                return false;
            }
        }
        ...
        {
            Console.WriteLine("BringUpExplorer({0}) called", directory);
            try
            {
                if (string.IsNullOrEmpty(directory))
                {
            ...
            {
                Console.WriteLine(string.Format("Unable to bring up Explorer in directory {0}", directory), e);
                return false;
            }
        }
        ...
        public static bool ShowRegistryEditor(string key)
        {
            ...
            {
                Console.WriteLine(string.Format("Unable to bring up Registry Editor in {0}", key), e);
                return false;
            }
        }
