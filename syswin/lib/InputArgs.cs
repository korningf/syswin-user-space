...
namespace syswin
{
    ...
    public class InputArgs
    {
        ...
        internal class InputArg
        {
            ...
            internal InputArg(InputArgType type, char opt, string name, object defaultValue, Presence presence, string helpString)
            {
                HasBeenSeen = false;
				Type = type;
				Opt = opt;
        ...
        public void Add(InputArgType type, char opt, string name, object defaultvalue, Presence presence, string helpString)
        {
            Options.Add(new InputArg(type, opt, name, defaultvalue, presence, helpString));
        }
		...
		private bool MatchesOption(char opt, InputArg option)
		{
        ...
        private bool DefaultProcessFunc(string arg)
        {
			...
			{
				char opt = (arg.Substring(1)).ToCharArray()[0];
				if (opt == '?' || opt == 'h' )
				{
					return Help();
				}

				InputArg option = GetOptionByChar (opt);
				arg = option.Name;
			}
			...
			if (arg.Equals ("?") || arg.Equals ("h") || arg.Equals ("help", SCType))
			{
				return Help ();
			}
...
+             */
        ...
        private bool Help()
        {
            ...
            foreach (InputArg arg in Options)
            {
                if( arg != remainingParameters )
					Console.WriteLine("{0,16}: {1}", "-" + arg.Opt + "/" + arg.Name.ToLower(), " " + string.Format(arg.HelpString, arg.Value));
            }
            if( remainingParameters != null )
				Console.WriteLine("{0,16}: {1}", " " + remainingParameters.Name, remainingParameters.HelpString);

            return false;
        }

		private InputArg GetOptionByChar(char opt)
		{
			foreach (InputArg option in Options)
			{
				if (MatchesOption(opt, option))
				{
					return option;
				}
			}
			throw new KeyNotFoundException(string.Format("Argument '{0}' is undefined", opt));
		}
		...
		public char GetCharacter(string argName)
		{
			return GetString(argName).ToCharArray()[0];
		}

		public int GetInteger(string argName)
		{
			return (int) GetValue(argName);
		}
